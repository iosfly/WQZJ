...
-(void)selectDate:(int)date {
    ...
    if ([delegate respondsToSelector:@selector(calendarView:dateSelected:)]){
        ...
        [delegate calendarView:self dateSelected:[self.selectedDate dateByAddingTimeInterval:+interval]];
    }
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    [formatter setDateFormat:@"yyyy MM月"];
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    if (self.markedDates && !isSelectedDatePreviousMonth && !isSelectedDateNextMonth){
        ...
        for (int i = 0; i<[self.markedDates count]; i++) {
            ...
            if ([markedDateObj intValue] == [[NSDate date] day] && hasToday == NO) {
                hasToday = YES;
            }
            ...
            int targetX = targetColumn * (kVRGCalendarViewDayWidth+2) + 0.5;
            int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2) + 0.5;
            
            //        CGRect rectangle = CGRectMake(targetX,targetY,32,2);
            CGRect rectangle = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth+1,kVRGCalendarViewDayHeight+1);
            CGContextAddRect(context, rectangle);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i == selectedDateBlock) {
            ...
            for (int j = 0; j<[self.markedDates count]; j++) {
                int markedDateIndex = [[self.markedDates objectAtIndex:j] intValue];
                if (i - 2 == markedDateIndex) {
                    isClear = YES;
                    colorIndex = j;
                    break;
                }
            ...
            UIColor * fillColor;
            if (isClear) {
                fillColor = [self.markedColors objectAtIndex:colorIndex];
            }else{
